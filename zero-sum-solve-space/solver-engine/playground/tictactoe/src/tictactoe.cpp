...
tictactoe::tictactoe(t3board initialState) : state(initialState) {}
...
// Return true if x's turn and false if o's turn
bool turn(t3board board) {
...
void tictactoe::printState() const {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        std::cout << "     | " << conv[3*i]  << " | " << conv[3*i+1] << " | " << conv[3*i+2] << " | " << std::endl;
        std::cout << "     -------------" << std::endl;
    }
    std::cout << std::endl;
}
...
// Previous implementation will not accept is. Only works for naive exhaustive searching
// Need to actual track left and right players
